...
int main() {
  ...
  // 2) STEREO independence: a voice with L != R stays separate
  ...
  // 6) WAV header parse (mono PCM16 22050 Hz, 1000 data bytes)
  uint8_t h[44] = {0};
  ...
  CHECK(w.ok && w.rate == 22050 && w.bits == 16 && w.channels == 1, "mono WAV header parsed");

  // 7) WAV parse recognises a STEREO file
